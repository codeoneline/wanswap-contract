...
contract('WswapToken', ([alice, bob, carol]) => {
    beforeEach(async () => {
        ...
        this.bar = await WswapBar.new(this.wswap.address, { from: alice });
        this.wswap.mint(alice, '100', { from: alice });
        this.wswap.mint(bob, '100', { from: alice });
        this.wswap.mint(carol, '100', { from: alice });
    });
    ...
    it('should not allow enter if not enough approve', async () => {
        ...
        await this.wswap.approve(this.bar.address, '50', { from: alice });
        await expectRevert(
            this.bar.enter('100', { from: alice }),
            'ERC20: transfer amount exceeds allowance',
        );
        await this.wswap.approve(this.bar.address, '100', { from: alice });
    ...
    it('should work with more than one participant', async () => {
        await this.wswap.approve(this.bar.address, '100', { from: alice });
        await this.wswap.approve(this.bar.address, '100', { from: bob });
        ...
        assert.equal((await this.wswap.balanceOf(this.bar.address)).valueOf(), '30');
        // WswapBar get 20 more WSWAPs from an external source.
        await this.wswap.transfer(this.bar.address, '20', { from: carol });
        // Alice deposits 10 more WSWAPs. She should receive 10*30/50 = 6 shares.
        ...
        assert.equal((await this.wswap.balanceOf(this.bar.address)).valueOf(), '52');
        assert.equal((await this.wswap.balanceOf(alice)).valueOf(), '70');
        assert.equal((await this.wswap.balanceOf(bob)).valueOf(), '98');
    });
});
