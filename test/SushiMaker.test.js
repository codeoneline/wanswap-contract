...
contract('WswapMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.wswap = await WswapToken.new({ from: alice });
        await this.wswap.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await WswapMaker.new(this.factory.address, bar, this.wswap.address, this.weth.address);
        this.wswapWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.wswap.address)).logs[0].args.pair);
    ...
    it('should make WSWAPs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.wswapWETH.address, '10000000', { from: minter });
        await this.wswap.transfer(this.wswapWETH.address, '10000000', { from: minter });
        await this.wswapWETH.mint(minter);
        ...
        // After calling convert, bar should have WSWAP value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.wswap.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for WSWAP-ETH pair
        await this.wswap.transfer(this.wswapWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.wswapWETH.address, '100000', { from: minter });
        await this.wswapWETH.sync();
        await this.wswap.transfer(this.wswapWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.wswapWETH.address, '10000000', { from: minter });
        await this.wswapWETH.mint(minter);
        assert.equal((await this.wswapWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.wswap.address, this.weth.address);
        assert.equal((await this.wswap.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.wswapWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});